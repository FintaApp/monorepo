...
export default wrapper(async ({ req, logger }) => {
  ...
  const syncData = { trigger, triggerDestinationId: destination.id, userIdOld: destination.userId, userIdNew: destination.userId, metadata: { targetTable: Table.Transactions } as SyncMetadata }
