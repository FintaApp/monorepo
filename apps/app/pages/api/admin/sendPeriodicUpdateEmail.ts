...
import { getCustomer, getCustomerSubscription } from "~/lib/stripe";
...
export default publicFunctionWrapper(async ({ req, logger }) => {
  ...
  if ( !userId ) { return { status: 400, message: "Missing userId" }};
...
const getTimePeriodString = (dateMoment: moment.Moment, frequency: Frequency ) => {
...
const getConfig = ({ frequency, timezone }: { frequency: Frequency, timezone: string }) => {
  ...
  return {
    sendNextPeriodicUpdateEmailAt: getSendNextPeriodicUpdateEmailAt({ frequency, timezone, forceNextDuration: true }),
