...
export const Account = ({ account }: { account: RouterOutput['plaid']['getPlaidItem']['accounts'][0] }) => {
  ...
  const uniqueIntegrations = _.uniq(account.destinations.map(destination => destination.integration))
      ...
      <Td py = "0" display = {{ base: "none", md: "table-cell" }}>
        <HStack>
          { uniqueIntegrations.map(integration => (
            <Box key = { integration } p = "1" rounded = "full" shadow = { mode('sm', 'dark.sm') }>
